...
public class MenuServiceImpl implements MenuService {
    ...
    public List<Map> findAllMenu() {
        //方式一：查询parent_id=0的的列表（一级菜单），循环得到每个一级菜单id，再拿者查询二级菜单，嵌套循环得到每个二级菜单id，再查询三级菜单
        //但是不推荐，因为和数据库交互次数：1+一级菜单数量+二级菜单数量

        //方式二：首先把符合条件的菜单查询出来（列表），再通过内存判断筛选出符合条件的记录（每一级的菜单列表）
    ...
    private List<Map> findMenuListByParentId(List<Menu> menuList, String parentId) {
        ...
        for (Menu menu : menuList) {
            if (menu.getParentId().equals(parentId)) {
                ...
                map.put("children", findMenuListByParentId(menuList, menu.getId()));//递归调用，查询子节点
            }
        }
        return mapList;
    }
