...
public class SpecServiceImpl implements SpecService {
    ...
    public void delete(Integer id) {
        ...
        template.setSpecNum(template.getSpecNum() - 1);
        templateMapper.updateByPrimaryKey(template);
        specMapper.deleteByPrimaryKey(id);  //这行必须写在最后面，因为写前面已经删完了，没法操作了
    }

    @Override
    //    通过分类名查询规格列表，放入缓存。自己写的
    public void saveSpecToRedisByCategory(String categoryName) {
    ...
    @Override
    //    所有通过分类名查询规格列表，放入缓存。自己写的
    public void saveAllSpecToRedisByCategory() {
        List<Category> categories = categoryMapper.selectAll();
        for (Category category : categories) {
            saveSpecToRedisByCategory(category.getName());
        }
    }
