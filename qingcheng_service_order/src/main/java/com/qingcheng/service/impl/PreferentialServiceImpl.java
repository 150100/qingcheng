...
public class PreferentialServiceImpl implements PreferentialService {
    ...
    @Override
    //通过 分类Id 和 消费金额 查询 优惠金额
    public int findPreMoneyByCategoryId(Integer categoryId, int money) {
        //查询条件：state：1（有效），type：1（不翻倍），开始时间，结束时间，结果按消费额降序
        Example example = new Example(Preferential.class);
        Example.Criteria criteria = example.createCriteria();
        criteria.andEqualTo("categoryId", categoryId);
        criteria.andLessThanOrEqualTo("buyMoney", money); //buyMoney<=money，buyMoney是满200减50的“200”，money是实际消费额
        criteria.andEqualTo("state", "1");
        criteria.andLessThanOrEqualTo("startTime", new Date());
        criteria.andGreaterThanOrEqualTo("endTime", new Date());
        //注意：以上都是用属性作比较，下面排序这个是用表字段！所以用驼峰
        example.setOrderByClause("buy_money desc"); //按消费额降序
        ...
        if (preferentials.size() >= 1) {
            ...
            if ("1".equals(preferential.getType())) { //如果不翻倍
                return preferential.getPreMoney(); //优惠金额
            } else { //翻倍
